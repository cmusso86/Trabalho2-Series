...
prev_fit_2_aj <- prev_fit_2$mean %>% InvBoxCox(lambda_auto)
LI_aj <- prev_fit_2$lower %>% InvBoxCox(lambda_auto)
LS_aj <- prev_fit_2$upper %>% InvBoxCox(lambda_auto)

prev_fit_2_copia <- prev_fit_2
prev_fit_2_copia$mean <- prev_fit_2_aj
prev_fit_2_copia$lower <- LI_aj
prev_fit_2_copia$upper <-  LS_aj
prev_fit_2_copia$x <- serie

df2 <- data.frame(prev_fit_2_aj, LI_aj, LS_aj)
colnames(df2) <- c("Previsão Pontual", "LI 95", "LS 95")
xtable(df2)
prev_fit_2_copia %>%  plot(main= "Previsão de ARIMA(1,1,2)(1,0,0)[12] com Box-Cox", cex.main=0.9, cex.axis=0.8)
...
prev_fit1_box_aj <- prev_fit1_box$mean %>% InvBoxCox(lambda_auto)
LI_aj <- prev_fit1_box$lower %>% InvBoxCox(lambda_auto)
LS_aj <- prev_fit1_box$upper %>% InvBoxCox(lambda_auto)

prev_fit1_box_copia <- prev_fit1_box
prev_fit1_box_copia$mean <- prev_fit1_box_aj
prev_fit1_box_copia$lower <- LI_aj
prev_fit1_box_copia$upper <-  LS_aj
prev_fit1_box_copia$x <- serie

df4 <- data.frame(prev_fit1_box_aj, LI_aj, LS_aj)
colnames(df4) <- c("Previsão Pontual", "LI 95", "LS 95")
xtable(df4)
prev_fit1_box_copia %>% plot(main= "Previsão de ETS(A,A,A) com Box-Cox", cex.main=0.9, cex.axis=0.8)
...
ar_bc <- mean(abs(dados_teste - prev_fit_2_aj)) # Nao é possivel usar a funcao accurary, entao calcula-se o erro assim
ets <- accuracy(prev_fit16, dados_teste)[[6]]
ets_bc <- mean(abs(dados_teste - prev_fit1_box_aj)) 
